...
class TestReplyProcessor(unittest.TestCase):
    ...
    def test_sent_command(self, mock_getenv, mock_get_github_repo):
        ...
        def mock_env(key):
            if key == "ISSUE_NUMBER": return "1"
            if key == "COMMENT_BODY": return "/sent"
            return None
    ...
    def test_rejected_command(self, mock_getenv, mock_get_github_repo):
        ...
        def mock_env(key):
            if key == "ISSUE_NUMBER": return "1"
            if key == "COMMENT_BODY": return "/rejected"
            return None
        mock_getenv.side_effect = mock_env

        reply_processor.main()

        mock_issue.remove_from_labels.assert_called_once_with("lead-new", "lead-sent", "follow-up-needed")
    ...
    def test_callback_command(self, mock_getenv, mock_get_github_repo):
        ...
        def mock_env(key):
            if key == "ISSUE_NUMBER": return "1"
            if key == "COMMENT_BODY": return "/callback"
            return None
        mock_getenv.side_effect = mock_env
    ...
    def test_followed_up_command(self, mock_getenv, mock_get_github_repo):
        ...
        def mock_env(key):
            if key == "ISSUE_NUMBER": return "1"
            if key == "COMMENT_BODY": return "/followed-up"
            return None
        mock_getenv.side_effect = mock_env
