...
def main():
    ...
    for issue in issues:
        ...
        # For now, use issue creation date as proxy for 'sent' if no other info
        if issue.created_at < seven_days_ago:
            if "follow-up-needed" not in [lead.name for lead in issue.labels]:
                issue.add_to_labels("follow-up-needed")
                issue.create_comment(
                    "It's been 7 days since this lead was marked as 'sent'. "
                    "Consider follow-up. Use `/followed-up` to mark done."
                )
