...
def load_template(name):
    path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "templates", f"{name}.md"
    )
...
def main():
    csv_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "data", "casting_leads.csv"
    )
    ...
    for issue in issues:
        ...
        if matching_lead:
            email_draft = (
                fill_template(email_tmpl, matching_lead)
                if email_tmpl
                else "Warning: email_template.md not found."
            )
            dm_draft = (
                fill_template(dm_tmpl, matching_lead)
                if dm_tmpl
                else "Warning: dm_template.md not found."
            )
