...
class TestGenerateDrafts(unittest.TestCase):
    ...
    def test_load_template_not_exists(self, mock_print, mock_exists):
        ...
        mock_print.assert_called_once_with("Warning: Template dummy_template.md not found.")
    ...
    def test_main(self, mock_load_template, mock_get_csv_data, mock_get_github_repo):
        ...
        mock_get_csv_data.return_value = [
            {'id': '1', 'project': 'Awesome Project', 'role': 'Lead Role', 'name': 'Actor1'},
            {'id': '2', 'project': 'Another Project', 'role': 'Lead Role', 'name': 'Actor2'}
        ]
...
class TestGenerateDraftsMain(unittest.TestCase):
    ...
    @patch(f"{generate_drafts.__name__}.get_csv_data")
    @patch(f"{generate_drafts.__name__}.load_template")
    def test_main_missing_template(self, mock_load_template, mock_get_csv_data, mock_get_github_repo):
        mock_repo = MagicMock()
        mock_get_github_repo.return_value = mock_repo
    ...
    @patch(f"{generate_drafts.__name__}.get_csv_data")
    @patch(f"{generate_drafts.__name__}.load_template")
    def test_main_missing_template2(self, mock_load_template, mock_get_csv_data, mock_get_github_repo):
        mock_repo = MagicMock()
        mock_get_github_repo.return_value = mock_repo
